...
export class ArCanvasComponent implements OnInit {
  ...
  @Input() onARLoaded: (arContext: ARContext) => Promise<void> =
    async (): Promise<void> => {
      // dummy that will be replaced by parent component
    };
  @Input() onARFrame: (arFrameEvent: ARFrameEvent) => Promise<void> =
    async (): Promise<void> => {
      // dummy that will be replaced by parent component
    };
  ...
  public async activateXR() {
    ...
    const webGLContext = this._canvas.getContext('webgl', {
      xrCompatible: true,
    });
    if (!webGLContext)
      throw new Error('Unable to load WebGL rendering context');
    ...
    const sessionRequest = window.navigator.xr.requestSession(this.xrMode, {
      requiredFeatures: this.xrRequiredFeatures,
    });
    if (!sessionRequest)
      throw new Error('Could not request immersive-ar session!');
  ...
  private async onXRFrame(time: number, frame: XRFrame): Promise<void> {
    const { session, webGL, renderer, space, camera, scene } = this._context;
    // Queue up the next draw request.
    session.requestAnimationFrame((t, f) => this.onXRFrame(t, f));

    // Bind the graphics framebuffer to the baseLayer's framebuffer
    if (!session.renderState.baseLayer)
      throw new Error('Unable to find base layer');
